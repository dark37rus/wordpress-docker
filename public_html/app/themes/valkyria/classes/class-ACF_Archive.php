...
class ACF_Archive
{
    ...
    public function boot()
    {
        if (!class_exists('ACF')) {
            add_action('admin_notices', [$this,
                                         'acf_installed_notify']);
            return;
        }

        add_action('admin_menu', [$this,
                                  'add_archive_option_page'], 20);
        add_action('admin_enqueue_scripts', [$this,
                                             'admin_enqueue_scripts']);
        add_action('acf/input/admin_footer', [$this,
                                              'admin_footer'], 10, 1);
        add_filter('acf/location/rule_types', [$this,
                                               'location_rules_types']);
        add_filter('acf/location/rule_values/admin_page', [$this,
                                                           'location_rules_values_archive']);
        add_filter('acf/location/rule_match/admin_page', [$this,
                                                          'location_rules_match_archive'], 10, 3);
    }
    ...
    {
        $page_name = sprintf(__('%s настройки', 'acf-archive'), $label);
    ...
    public function render_menu()
    {
        ...
        <div class="wrap">
            <h1><?= __($post_type_object->label) ?></h1>
            <?php SiteLang::translate_string($post_type_object->label); ?>
            <form action="" method="post">
                ...
                <p class="submit">
                    <button class="button button-primary" type="submit"
                            name="<?php echo $post_type; ?>"><?php echo __('Update', 'acf-archive'); ?></button>
                </p>
            </form>
        </div>
